...
dlcm        = LinearSegmentedColormap.from_list("dlcm", dlcolors)

##########################################################
# Plotting and computation routines
##########################################################
...
def plt_house_x(X, y, f_wb=None, ax=None):
    """Plot house data with optional prediction"""
    if ax is None:
...
def mk_cost_lines(x, y, w, b, ax):
    """Make vertical cost lines for visualization"""
    cstr = "cost = (1/m)*("
    ctot = 0
...
def plt_stationary(x_train, y_train):
    """2D cost curve with interactive slider"""
    ...
    ax[2].xaxis.set_pane_color((1, 1, 1, 0))
    ax[2].yaxis.set_pane_color((1, 1, 1, 0))
    ax[2].zaxis.set_pane_color((1, 1, 1, 0))
...
class plt_update_onclick:
    ...
    def __call__(self, event):
        if event.inaxes == self.ax[1]:
            ...
            # remove lines and re-add on contour plot
